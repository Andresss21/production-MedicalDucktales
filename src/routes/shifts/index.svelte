...
<script lang="ts">
  ...
  let result = null
    ...
    const url = variables.apiUri + `/shifts/${idToDelete}`;
...
<div class="card bg-neutral card-bordered">
  <div class="card-body text-neutral-content">
    <div class="card-title">
      ...
      <a href="/shifts/new" class="btn btn-accent ml-auto">
        <Icon
          name="plus"
          width="1.5em"
          height="1.5em"
          ...
          <tr>
            ...
            <td>
              {#each Object.entries(row.days) as [key, value]}
                {#if value}
                  <span class="bg-cyan-200 px-2 py-2 mx-1 rounded-2xl capitalize">
                    {value ? key : ''}
                  </span>
                {/if}
              {/each}
